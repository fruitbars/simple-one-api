...
号池的目标不是把请求机械地平均分配，而是在每次请求前判断“哪个 Key 现在真正有能力接收这次请求”。这对长上下文、Codex Responses、工具调用和批量任务尤其重要：单个 Key 的 TPM 可能很高，但一次请求也可能消耗掉它大部分一分钟窗口。

一次请求的 Key 选择顺序如下：

1. 从 `credential_list` 中移除 `enabled: false` 的 Key，并跳过当前 Provider/Key/模型熔断中的组合。
2. 根据请求体估算 Token 成本。估算会覆盖聊天消息、工具定义、`max_tokens`/输出预算、Responses 原始输入和 Embedding 输入；这是限流用的保守近似，不是供应商 tokenizer 的精确账单。
3. 读取每个 `Key + 模型` 的 60 秒滚动预留窗口和 429 冷却时间。先排能容纳本次成本的 Key，再按剩余容量从高到低排序；都无法立即容纳时，按预计最早恢复时间排序。
4. 在同一排序组内保留 `load_balancing` 的基础顺序（`first`、`random`、`round_robin` 或 `hash`），因此仍然具备可预测的基础分布策略。
5. Key 通过组合限流器后才写入 Token 预留。预留不会因为上游提前返回而退回，避免本地估计低于供应商实际计量。
6. 如果上游返回 429，当前 `Key + 模型` 默认进入 30 秒冷却；如果网关还没有向客户端写出响应，会继续尝试下一个健康 Key。已经开始输出的流式响应不会中途换 Key，以避免拼接出不一致的结果。

四层限制的作用范围如下：

| 层级 | 计量范围 | 换 Key 是否能增加该层容量 |
| --- | --- | --- |
| Provider | 同一个 Provider 的所有模型和 Key | 否，所有 Key 共享 |
| Provider-模型 | 同一个 Provider 下的指定模型 | 否，同模型的所有 Key 共享 |
| Key | 一个 API Key 的所有模型 | 是，可以切换到池内其他 Key |
| Key-模型 | 一个 API Key 调用指定模型 | 是，可以切换到池内其他 Key |

每一层的 `qps`、`qpm`、`rpm`、`tpm`、`concurrency` 和 `timeout` 都可以独立填写；同一层填写多个字段时是“同时满足”，不是互相覆盖。例如，一个 Key 可以同时限制 `qps: 2`、`tpm: 1000000` 和 `concurrency: 2`。Provider 总限制仍然会叠加在号池之上，因此增加 Key 不会绕过你设置的 Provider 级总容量。

### 运行状态和排障

配置台 Provider 页面会轮询 `GET /api/admin/capacity`，按模型显示每个 Key 的 TPM 上限、已预留 Token、剩余容量、可用/冷却/等待窗口状态和预计恢复时间。该接口只返回稳定 ID、显示名称和数值状态，不返回 `api_key`。

看到 429 时，可以按下面顺序排查：

- 先看是否所有 Key 都处于“冷却中”或“等待窗口”；这通常表示请求本身接近单 Key 的 TPM 上限，需要更多 Key 或降低单次上下文/输出预算。
- 再看 Provider 或 Provider-模型共享限制；如果共享层达到上限，切换 Key 不会改善结果。
- 对照上游返回的 `Retry-After`、模型限流说明和计费控制台。网关的 Token 估算是保守近似，不能替代供应商的真实限流规则。
- 确认每个 Key 的 `id` 稳定且唯一、`enabled` 为 `true`，并且模型名称与客户端请求的模型一致；模型别名通过 `model_map` 映射后仍按实际路由模型计量。

运行时预留和冷却只存在于当前 Go 进程内存中，不会写入 SQLite，也不会跨进程共享。重启网关后窗口会重新开始；这意味着状态页面适合观察调度压力，不适合作为供应商余额或历史用量报表。
...
Wails App 启动时会在 `127.0.0.1:<server_port>` 启动与服务端相同的 API 网关，默认地址为 `http://127.0.0.1:9090`。本地 OpenAI 兼容客户端可使用 `http://127.0.0.1:9090/v1` 作为 Base URL，并通过网关主 `api_key` 鉴权。`enable_web: true` 时，loopback 网关也提供 `/`、`/admin`、`/chat` 和内嵌静态资源；浏览器配置台会提示输入主密钥并只保存到当前 `sessionStorage`。关闭 App 会停止网关并释放端口；如果端口已被其他进程占用，桌面 UI 仍可运行，但外部客户端无法连接该 App 的网关。
